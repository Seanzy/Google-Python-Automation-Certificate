...
def current_users(events):
  ...
  for event in events:
#     print(event.date, machines)
    if event.machine not in machines:
      machines[event.machine] = set()
#       print("Added", machines)
    if event.type == "login":
      machines[event.machine].add(event.user)
#       print("login", machines)
#     elif event.machine != set([]):
#         print("TRUEE", event.machine)
    elif event.type == "logout":
#       print("Logout", event.machine, machines)
      if machines[event.machine] != set([]): 
#         print("TRUE, removing ", event.user, "from", event.machine)
